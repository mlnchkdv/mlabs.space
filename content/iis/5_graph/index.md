...
- **Количество вершин**: любой граф может быть характеризован целым числом $n$ – количество вершин. $\abs{V} = n$
- **Количество ребер**: граф может быть описан как имеющий $m$ ребер, где $m$ – целое число. $\abs{E} = m$
- **Плотность (Density)**: мера, которая показывает насколько «плотно» или «разреженно» заполнен граф, т.е. <mark>на сколько близко количество ребер в графе к максимально возможному количеству ребер</mark>. Плотность графа определяется как отношение числа ребер к максимально возможному числу ребе. Для неориентированного графа с вершинами $V$, максимальное количество ребер равно $\frac{\abs{V} \times (\abs{V}-1)}{2}$, тогда плотность графа определяется как $D=\frac{2 \abs{E}}{\abs{V} \times (\abs{V}-1)}$. В то время как для ориентированного графа плотность графа определяется как $D=\frac{\abs{E}}{\abs{V} \times (\abs{V}-1)}$. <mark>Граф, имеющий малое число рёбер, принято называть разреженным графом</mark>.
